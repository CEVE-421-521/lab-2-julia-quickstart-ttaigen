...
march_2020_data = df[(df[!, :datetime] .>= DateTime(2020, 3, 1)) .& (df[!, :datetime] .< DateTime(2020, 4, 1)), :]

# Plotting hourly water levels
plot(march_2020_data[!, :datetime], march_2020_data[!, :lsl], xlabel="Date", ylabel="Water Level (m)", label="Hourly Water Levels for March 2020", legend=:topleft)

```

```{julia} 
# Group by month and calculate the mean water level
monthly_avg_data = combine(groupby(df, Dates.monthname.(df[!, :datetime])), :lsl => mean => :avg_lsl)

# Plotting average monthly water levels
plot(monthly_avg_data[!, :month], monthly_avg_data[!, :avg_lsl], xlabel="Month", ylabel="Average Water Level (m)", label="Average Monthly Water Levels", legend=:topleft)

```

```{julia}
# Avg Daily Water Level (ALL YEARS)
# Group by day of the year and calculate the mean water level
daily_avg_data = combine(groupby(df, Dates.dayofyear), :lsl => mean => :avg_lsl)

# Plotting average daily water levels
plot(daily_avg_data[!, :dayofyear], daily_avg_data[!, :avg_lsl], xlabel="Day of the Year", ylabel="Average Water Level (m)", label="Average Daily Water Levels", legend=:topleft)
```